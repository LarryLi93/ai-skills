...
- 图片：实拍与创意图片，默认使用 Gpt Image 2.5，质量固定默认为 `low`
...
The script stores the key in `~/.openflowly/config.json`, immediately fetches `GET /v1/config?all=false`, and stores that response in `~/.openflowly/model_config.json`. Both files use restrictive permissions. Prefer the environment variable when available. The default API base is `https://www.openflowly.com/v1`; override it with `OPENFLOWLY_BASE_URL` or `--base-url` only when the user explicitly provides another endpoint. Local reference media must always be uploaded through the public endpoint `https://www.openflowly.com/v1/upload/`, regardless of the generation API base, so the returned media URL is reachable by the generation service.

## Defaults

Unless the user selects another model, use:

- Images: `Gpt Image 2.5`, `resolution=1K`, `quality=low`. When the user does not explicitly specify quality, always send `quality=low`; an explicit user value may override it.
...
5. Before downloading, create a fresh unique batch directory under the current project's `outputs/`, for example `outputs/openflowly-YYYYMMDD-HHMMSS-<nonce>/`. Never search the whole `outputs/` directory for the newest image, never reuse fixed names such as `scene-01.png`, and never display a file from an earlier batch. Do not write a transient `tasks.jsonl` or task-result JSON file into this batch directory; per-task state lives only in process memory and in stdout JSON lines, so success, explicit failure, `poll_timeout`, and `poll_error` leave no JSON task residue. Only downloaded media files should be placed in the batch directory.
6. Download each successful media URL directly into that batch directory using a filename containing both the 1-based prompt index and the returned `task_id`. The script must return `local_output_path`; verify the file exists, is non-empty, and resolves to the newly created batch directory before displaying it. If a URL is missing, do not claim success or invent a local path; report a failed JSON result and continue without leaving a task record.
7. Display only the returned absolute `local_output_path` values from the current command's JSON results, preserving index-to-prompt order. Do not infer scene identity from directory listings, timestamps, prior filenames, or URL basenames.
8. Return a concise batch table with prompt, model, task ID, status, external media URL, and local output path. If the task is still pending after 20 polls, report timeout and retain the task ID for manual follow-up.
