...
import argparse, json, mimetypes, os, stat, sys, time, ssl
...
DEFAULTS = {"image": {"model": "Gpt Image 2", "resolution": "1K", "quality": "low"},
...
def prepare_reference_fields(kind, args, model_config):
    ...
    subject_urls += [upload_file(path, args.base_url, args.api_key) for path in subject_paths]
    if args.first_frame_image: first_frame = upload_file(args.first_frame_image, args.base_url, args.api_key)
    if args.end_frame_image: end_frame = upload_file(args.end_frame_image, args.base_url, args.api_key)
    return route_reference_fields(kind, model_config, subject_urls, subject_videos,
                                  subject_audios, first_frame, end_frame)

def generate_one(kind, args, prompt, index):
    ...
    if direct: return {"index": index, "prompt": prompt, "model": body["model"], "task_id": task_id, "status": "succeeded", "url": direct}
    if not task_id: raise RuntimeError("Generation response did not contain a task ID or media URL")
    for attempt in range(1, 21):
        time.sleep(10)
        result = request_json(f"{args.base_url.rstrip('/')}/tasks/{task_id}?include_media_data=true", args.api_key, headers={"X-Model-Name": body["model"]})
        status = task_status(result)
        direct = media_url(result)
        if direct or status in {"completed", "succeeded", "success", "done", "finish", "finished"}:
            embedded = first_value(result, ("media_data_url",))
            return {"index": index, "prompt": prompt, "model": body["model"], "task_id": task_id, "status": "succeeded", "url": direct, "media_data_url": embedded}
        if status in {"failed", "error", "fail", "cancelled", "canceled", "timeout"}:
            return {"index": index, "prompt": prompt, "model": body["model"], "task_id": task_id, "status": status, "url": None}
    return {"index": index, "prompt": prompt, "model": body["model"], "task_id": task_id, "status": "poll_timeout", "url": None}
...
def run_generate(args):
    ...
    for i, prompt in enumerate(expanded, 1):
        try: results.append(generate_one(args.kind, args, prompt, i))
        except Exception as exc: results.append({"index": i, "prompt": prompt, "status": "error", "error": str(exc)})
