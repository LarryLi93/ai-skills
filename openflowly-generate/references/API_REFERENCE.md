...
- `GET /tasks/{task_id}?include_media_data=true`: send `X-Model-Name: <the model name used for submission>`. The response adds `b64_json` and `media_data_url` beside each remote media URL.

The same `include_media_data=true` request field is accepted by both generation endpoints for synchronous provider responses. Video data is returned with its original MIME type, so clients can use the `data:video/...;base64,...` value directly or decode `b64_json` to an MP4 file.
